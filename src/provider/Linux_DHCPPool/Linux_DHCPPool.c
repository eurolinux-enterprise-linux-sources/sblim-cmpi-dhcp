...
#ifndef CMPI_VER_100
#define Linux_DHCPPool_ModifyInstance Linux_DHCPPool_SetInstance
#endif
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_EnumInstanceNames(
            ...
            const CMPIObjectPath * reference)    /** [in] Contains target namespace and classname. */
{
    ...
    if ( !Pool_isEnumerateInstanceNamesSupported() ) {
    ...
    ra_status = Linux_DHCPPool_getResources( &resources );
    ...
    ra_status = Linux_DHCPPool_getNextResource( resources, &resource );
    while ( ra_status.rc == RA_RC_OK && resource ) {
        ...
        ra_status = Linux_DHCPPool_setInstanceFromResource( resource, instance, _BROKER );
        if ( ra_status.rc != RA_RC_OK ) {
        ...
        ra_status = Linux_DHCPPool_freeResource( resource );
        if ( ra_status.rc != RA_RC_OK ) {
    ...
    ra_status = Linux_DHCPPool_freeResources( resources );
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_EnumInstances(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference,    /** [in] Contains target namespace and classname. */
            const char ** properties)            /** [in] List of desired properties (NULL=all). */
{
    ...
    if ( !Pool_isEnumerateInstancesSupported() ) {
        build_cmpi_error_msg ( _BROKER, &status, CMPI_RC_ERR_NOT_SUPPORTED, _("This function is not supported") );
        goto exit;
    }
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_GetInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            ...
            const char ** properties)            /** [in] List of desired properties (NULL=all). */
{
    ...
    ra_status = Linux_DHCPPool_getResourceForObjectPath( resources, &resource, reference );
    if ( ra_status.rc != RA_RC_OK ) {
        build_ra_error_msg ( _BROKER, &status, CMPI_RC_ERR_FAILED, _("Failed to get resource data"), ra_status );
        goto clean_on_error;
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_ModifyInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference,    /** [in] Contains the target namespace, classname and object path. */
            ...
            const char** properties)             /** [in] List of desired properties (NULL=all). */
{
   ...
   /** Parameters for the use of Indication*/
    ...
    ra_status = Linux_DHCPPool_setResourceFromInstance( &resource, newinstance, properties, _BROKER );
    if ( ra_status.rc != RA_RC_OK ) {
    ...
    indop = CMNewObjectPath( _BROKER, namespace, "Linux_DHCPPool_Ind", &status );
    ...
    CMSetProperty (indinst, "IndicationIdentifier", (CMPIValue *)"Linux_DHCPPool", CMPI_chars);
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_CreateInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference,    /** [in] Contains the target namespace, classname and object path. */
            const CMPIInstance * newinstance)    /** [in] Contains the new instance data. */
{
   ...
   /** Parameters for the use of Indication*/
    ...
    ra_status = Linux_DHCPPool_createResourceFromInstance( resources, &resource, newinstance, _BROKER );
    if ( ra_status.rc != RA_RC_OK ) {
    ...
    ra_status = Linux_DHCPPool_BuildObjectPath(objectpath, (CMPIInstance*)newinstance , (char*)namespace, resource);
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_DeleteInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference)  	 /** [in] Contains the target namespace, classname and object path. */
{
   ...
   /** Parameters for the use of Indication*/
    ...
    ra_status = Linux_DHCPPool_deleteResource( resources, resource, _BROKER );
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_ExecQuery(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
...
/// first loaded.
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPPool_Initialize(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context)         /** [in] Additional context info, if any. */
{
    CMPIStatus status = {CMPI_RC_OK, NULL};
...
/** Create a new CMPIObjectPath to store this resource. */
        ...
        CMAddKey (op, "Name", (CMPIValue *)"PoolIndication", CMPI_chars);
        CMAddKey (op, "SystemCreationClassName", (CMPIValue *)"CIM_ComputerSystem", CMPI_chars);
    
       /** Create a new CMPIInstance to store this resource. */

        instance = CMNewInstance( _BROKER, op, &status );
        CMSetProperty(instance, "Name", (CMPIValue *)"PoolIndication", CMPI_chars);
        char *v;
        v = "SELECT * FROM Linux_DHCPPool_Ind"; 
   ...
   //printf("Created an instance of PoolIndication Filter\n");

    /** Initialize instance provider */
    ra_status = Linux_DHCPPool_InstanceProviderInitialize( &ra_status);
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_DHCPPool_Cleanup(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            CMPIBoolean terminating)             /** [in] Switch to wether to teminate or not*/ 
{
    ...
    ra_status = Linux_DHCPPool_InstanceProviderCleanUp(lTerminating);
    if ( ra_status.rc != RA_RC_OK ) {
...
CMPIStatus Linux_DHCPPool_IndicationCleanup(
                                           CMPIIndicationMI * mi,
...
CMPIStatus Linux_DHCPPool_AuthorizeFilter(
                                           CMPIIndicationMI * mi,
                                           const CMPIContext * ctx,
...
CMPIStatus Linux_DHCPPool_MustPoll(
                                    CMPIIndicationMI * mi,
                                    const CMPIContext * ctx,
                                    const CMPISelectExp * se,
...
CMPIStatus Linux_DHCPPool_ActivateFilter(
                                          CMPIIndicationMI * mi,
                                          const CMPIContext * ctx,
                                          const CMPISelectExp * se,
                                          const char *ns,
...
CMPIStatus Linux_DHCPPool_DeActivateFilter(
                                            CMPIIndicationMI * mi,
                                            const CMPIContext * ctx,
                                            const CMPISelectExp * se,
                                            const char *ns,
...
CMPIStatus Linux_DHCPPool_EnableIndications(
                                              CMPIIndicationMI * mi,
                                              const CMPIContext * ctx)
{
    _enabled = 1;
...
CMPIStatus Linux_DHCPPool_DisableIndications(
                                               CMPIIndicationMI * mi,
                                               const CMPIContext * ctx)
{
...
/// ============================================================================
CMInstanceMIStub( Linux_DHCPPool_ , Linux_DHCPPoolProvider, _BROKER, Linux_DHCPPool_Initialize( &mi, ctx ) );
CMIndicationMIStub( Linux_DHCPPool_ , Linux_DHCPPoolProvider, _BROKER, CMNoHook );
