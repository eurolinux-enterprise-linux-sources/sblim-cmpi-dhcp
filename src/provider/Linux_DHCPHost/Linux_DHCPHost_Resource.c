...
bool Host_isEnumerateInstanceNamesSupported() { return true; };
bool Host_isEnumerateInstancesSupported()     { return true; };
bool Host_isGetSupported()                    { return true; };
bool Host_isCreateSupported()                 { return true; };
bool Host_isModifySupported()                 { return true; };
bool Host_isDeleteSupported()                 { return true; };
...
_RA_STATUS Linux_DHCPHost_getNextResource( _RESOURCES* resources, _RESOURCE** resource ) {
	...
	memset(temp, '\0', sizeof(_RESOURCE));
        ///ERROR CONDITION if Dyanmic memory allocation fails
        if( temp == NULL) {
                setRaStatus( &ra_status, RA_RC_FAILED,  DYNAMIC_MEMORY_ALLOCATION_FAILED, _("Dynamic Memory Allocation Failed") );
                return ra_status;
        }
...
_RA_STATUS Linux_DHCPHost_getResourceForObjectPath( _RESOURCES* resources, _RESOURCE** resource, const CMPIObjectPath* objectpath ) {
    ...
    for(itr = resources->Array, index = 0; *itr != NULL; index++, itr++){
	    ...
	    memset((*resource), '\0', sizeof(_RESOURCE));
                ///ERROR CONDITION if Dynamic memory allocation fails
                if( (*resource) == NULL) {
                        setRaStatus( &ra_status, RA_RC_FAILED, DYNAMIC_MEMORY_ALLOCATION_FAILED, _("Dynamic Memory Allocation Failed") );
                        return ra_status;
                }
...
_RA_STATUS Linux_DHCPHost_setInstanceFromResource( _RESOURCE* resource, const CMPIInstance* instance, const CMPIBroker* broker ) {
    ...
    CMSetProperty(instance, "ElementName", (CMPIValue *)"Host", CMPI_chars);
...
_RA_STATUS Linux_DHCPHost_setResourceFromInstance( _RESOURCE** resource, const CMPIInstance* instance, const char** properties, const CMPIBroker* broker ) {
    ...
    ///ERROR CONDITION - Dynamic memory allocation fails
    if(pnode == NULL) {
    ...
    Array = ra_getAllEntity(HOSTF, NULL, &ra_status);
    for(count = 0; Array[count]; count++){
	if((strcmp((char *)(Array[count]->obValue), value) == 0)){
	    if(pid == Array[count]->obID)
		continue;
	    setRaStatus( &ra_status, RA_RC_FAILED, ENTITY_ALREADY_EXISTS, _("A host with the name already exists") );
	    return ra_status;
	}
    }
...
_RA_STATUS Linux_DHCPHost_createResourceFromInstance( _RESOURCES* resources, _RESOURCE** resource, const CMPIInstance* instance, const CMPIBroker* broker ) {
    ...
    newnode = ra_createHost(strdup("host"), (char*)value, 0);
    if (newnode == NULL) {
...
_RA_STATUS Linux_DHCPHost_InstanceProviderInitialize( _RA_STATUS* ra_status){
    ra_Initialize( ra_status );
    return (*ra_status);
}

_RA_STATUS Linux_DHCPHost_InstanceProviderCleanUp(bool term){
    _RA_STATUS ra_status ={RA_RC_OK, 0, NULL};
    if(term) ra_CleanUp();
    return ra_status;
}
...
///----------------------------------------------------------------------------
_RA_STATUS Linux_DHCPHost_BuildObjectPath(CMPIObjectPath* objectpath, CMPIInstance* newinstance , char* namespace, _RESOURCE* resource) {
    _RA_STATUS ra_status ={RA_RC_OK, 0, NULL};
