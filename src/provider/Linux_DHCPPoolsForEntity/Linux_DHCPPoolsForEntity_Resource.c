...
int Linux_DHCPPoolsForEntity_isAssociated ( CMPIObjectPath * lhs, CMPIObjectPath * rhs)
{
...
_RA_STATUS Linux_DHCPPoolsForEntity_getResources( const CMPIBroker * broker, const CMPIContext * context, const CMPIObjectPath * reference, _RESOURCES ** resources, int returnResult)
{
	...
	for(i = 0; i < rhs_length; i++)
	{
	    ...
	    if (lhs_op && rhs_op && 
		    (state = Linux_DHCPPoolsForEntity_isAssociated(lhs_op, rhs_op)) &&
		    (state & returnResult)) {
...
_RA_STATUS Linux_DHCPPoolsForEntity_getResourceForObjectPath( const CMPIBroker * broker, const  CMPIContext * ctx, _RESOURCES * resources, _RESOURCE ** resource, const CMPIObjectPath * objectpath)
{
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
    CMPIStatus cmpi_status = {CMPI_RC_OK, NULL};
    CMPIData cmpiInfo;
