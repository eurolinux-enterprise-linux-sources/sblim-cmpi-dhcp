...
bool Group_isEnumerateInstanceNamesSupported() { return true; };
bool Group_isEnumerateInstancesSupported()     { return true; };
bool Group_isGetSupported()                    { return true; };
bool Group_isCreateSupported()                 { return true; };
bool Group_isModifySupported()                 { return false; };
bool Group_isDeleteSupported()                 { return true; };

/// ----------------------------------------------------------------------------

/** Get a handle to the list of all system resources for this class. */
_RA_STATUS Linux_DHCPGroup_getResources( _RESOURCES** resources  ) {
...
_RA_STATUS Linux_DHCPGroup_getNextResource( _RESOURCES* resources, _RESOURCE** resource ) {
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
    _RESOURCE * temp;
...
_RA_STATUS Linux_DHCPGroup_getResourceForObjectPath( _RESOURCES* resources, _RESOURCE** resource, const CMPIObjectPath* objectpath ) {
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
...
_RA_STATUS Linux_DHCPGroup_freeResource( _RESOURCE* resource ) {
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
    if(resource) {
...
_RA_STATUS Linux_DHCPGroup_freeResources( _RESOURCES* resources ) {
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
    if(resources) {
    	if(resources->Array) {
...
_RA_STATUS Linux_DHCPGroup_setInstanceFromResource( _RESOURCE* resource, const CMPIInstance* instance, const CMPIBroker* broker ) {
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
    char * parentID;
    ...
    CMSetProperty(instance, "ElementName", (CMPIValue *)"Group", CMPI_chars);
...
_RA_STATUS Linux_DHCPGroup_deleteResource( _RESOURCES* resources, _RESOURCE* resource, const CMPIBroker* broker ) {
...
_RA_STATUS Linux_DHCPGroup_setResourceFromInstance( _RESOURCE** resource, const CMPIInstance* instance, const char** properties, const CMPIBroker* broker ) {
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
    return ra_status;
}
...
_RA_STATUS Linux_DHCPGroup_createResourceFromInstance( _RESOURCES* resources, _RESOURCE** resource, const CMPIInstance* instance, const CMPIBroker* broker ) {
    _RA_STATUS ra_status = {RA_RC_OK, 0, NULL};
    CMPIStatus cmpi_status = {CMPI_RC_OK, NULL};
    CMPIData cmpi_info;
    NODE * pnode, * newnode;
    ...
    newnode = ra_createGroup(strdup("group"), 0);
   ...
   /// CMSetProperty(instance, "InstanceID", (CMPIValue *)idstring, CMPI_chars);

    return ra_status;
}

_RA_STATUS Linux_DHCPGroup_InstanceProviderInitialize( _RA_STATUS* ra_status){
    ra_Initialize( ra_status );
    return (*ra_status);
}

_RA_STATUS Linux_DHCPGroup_InstanceProviderCleanUp(bool term){
    _RA_STATUS ra_status ={RA_RC_OK, 0, NULL};
    if(term) ra_CleanUp();
    return ra_status;
}
...
///----------------------------------------------------------------------------
_RA_STATUS Linux_DHCPGroup_BuildObjectPath(CMPIObjectPath* objectpath, CMPIInstance* newinstance , char* namespace, _RESOURCE* resource) {
    _RA_STATUS ra_status ={RA_RC_OK, 0, NULL};
