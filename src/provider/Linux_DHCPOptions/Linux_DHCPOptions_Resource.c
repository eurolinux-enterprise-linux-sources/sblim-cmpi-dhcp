...
bool Option_isEnumerateInstanceNamesSupported() { return true; };
bool Option_isEnumerateInstancesSupported()     { return true; };
bool Option_isGetSupported()                    { return true; };
bool Option_isCreateSupported()                 { return true; };
bool Option_isModifySupported()                 { return true; };
bool Option_isDeleteSupported()                 { return true; };
...
_RA_STATUS Linux_DHCPOptions_getResources( _RESOURCES** resources  ) {
    ...
    ra_lockRaData();
    (*resources)->Array = ra_getAllEntity(OPTIONF | PARAMSF | SUPPORTF, NULL, &ra_status);
...
_RA_STATUS Linux_DHCPOptions_setInstanceFromResource( _RESOURCE* resource, const CMPIInstance* instance, const CMPIBroker* broker ) {
    ...
    CMSetProperty(instance, "ElementName", (CMPIValue *)"Options", CMPI_chars);
...
_RA_STATUS Linux_DHCPOptions_setResourceFromInstance( _RESOURCE** resource, const CMPIInstance* instance, const char** properties, const CMPIBroker* broker ) {
	...
	if( (strcasecmp(name, pnode->obName)) != 0 ) {
	    setRaStatus( &ra_status, RA_RC_FAILED, INVALID_INSTANCE_NAME, _("Modification of Option Name not permitted"));
	    return ra_status;
	}
    }
...
_RA_STATUS Linux_DHCPOptions_createResourceFromInstance( _RESOURCES* resources, _RESOURCE** resource, const CMPIInstance* instance, const CMPIBroker* broker ) {
	...
	if( (newnode->obFlags & (OPTIONF | PARAMSF)) && (strcmp(newnode->obName, name)) == 0){
	    setRaStatus( &ra_status, RA_RC_FAILED, ENTITY_ALREADY_EXISTS, _("A duplicate option already exists in this scope") );
	    return ra_status;
	}
    ...
    newnode = ra_createParam((char*)name, (char*)value, OPTIONF | PARAMSF | SUPPORTF, 0);
...
_RA_STATUS Linux_DHCPOptions_InstanceProviderInitialize( _RA_STATUS* ra_status ){
    ra_Initialize( ra_status);
    return (*ra_status);
}

_RA_STATUS Linux_DHCPOptions_InstanceProviderCleanUp(bool term){
    _RA_STATUS ra_status ={RA_RC_OK, 0, NULL};
    if(term) ra_CleanUp();
    return ra_status;
}
...
///----------------------------------------------------------------------------
_RA_STATUS Linux_DHCPOptions_BuildObjectPath(CMPIObjectPath* objectpath, CMPIInstance* newinstance , char* namespace, _RESOURCE* resource) {
    _RA_STATUS ra_status ={RA_RC_OK, 0, NULL};
