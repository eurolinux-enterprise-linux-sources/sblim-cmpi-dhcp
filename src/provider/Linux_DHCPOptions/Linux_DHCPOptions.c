...
#include "Linux_DHCPOptions_Resource.h"

#ifndef CMPI_VER_100
#define Linux_DHCPOptions_ModifyInstance Linux_DHCPOptions_SetInstance
#endif
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_EnumInstanceNames(
            ...
            const CMPIObjectPath * reference)    /** [in] Contains target namespace and classname. */
{
    ...
    if ( !Option_isEnumerateInstanceNamesSupported() ) {
    ...
    ra_status = Linux_DHCPOptions_getResources( &resources );
    ...
    ra_status = Linux_DHCPOptions_getNextResource( resources, &resource );
    while ( ra_status.rc == RA_RC_OK && resource ) {
        ...
        ra_status = Linux_DHCPOptions_setInstanceFromResource( resource, instance, _BROKER );
        if ( ra_status.rc != RA_RC_OK ) {
        ...
        ra_status = Linux_DHCPOptions_freeResource( resource );
        if ( ra_status.rc != RA_RC_OK ) {
    ...
    ra_status = Linux_DHCPOptions_freeResources( resources );
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_EnumInstances(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference,    /** [in] Contains target namespace and classname. */
            const char ** properties)            /** [in] List of desired properties (NULL=all). */
{
    ...
    if ( !Option_isEnumerateInstancesSupported() ) {
        build_cmpi_error_msg ( _BROKER, &status, CMPI_RC_ERR_NOT_SUPPORTED, _("This function is not supported") );
        goto exit;
    }
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_GetInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            ...
            const char ** properties)            /** [in] List of desired properties (NULL=all). */
{
    ...
    ra_status = Linux_DHCPOptions_getResourceForObjectPath( resources, &resource, reference );
    if ( ra_status.rc != RA_RC_OK ) {
        build_ra_error_msg ( _BROKER, &status, CMPI_RC_ERR_FAILED, _("Failed to get resource data"), ra_status );
        goto clean_on_error;
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_ModifyInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference,    /** [in] Contains the target namespace, classname and object path. */
            ...
            const char** properties)             /** [in] List of desired properties (NULL=all). */
{
   ...
   /** Parameters for the use of Indication*/
    ...
    ra_status = Linux_DHCPOptions_setResourceFromInstance( &resource, newinstance, properties, _BROKER );
    if ( ra_status.rc != RA_RC_OK ) {
    ...
    indop = CMNewObjectPath( _BROKER, namespace, "Linux_DHCPOptions_Ind", &status );
    ...
    CMSetProperty (indinst, "IndicationIdentifier", (CMPIValue *)"Linux_DHCPOptions", CMPI_chars);
    indStatus = CBDeliverIndication(_BROKER, context, (char*)namespace , (CMPIInstance*)indinst);
    if (indStatus.rc != CMPI_RC_OK)
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_CreateInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference,    /** [in] Contains the target namespace, classname and object path. */
            const CMPIInstance * newinstance)    /** [in] Contains the new instance data. */
{
   ...
   /** Parameters for the use of Indication*/
    ...
    ra_status = Linux_DHCPOptions_createResourceFromInstance( resources, &resource, newinstance, _BROKER );
    if ( ra_status.rc != RA_RC_OK ) {
    ...
    ra_status = Linux_DHCPOptions_BuildObjectPath(objectpath, (CMPIInstance*)newinstance , (char*)namespace, resource);
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_DeleteInstance(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
            const CMPIObjectPath * reference)  	 /** [in] Contains the target namespace, classname and object path. */
{
   ...
   /** Parameters for the use of Indication*/
    ...
    ra_status = Linux_DHCPOptions_deleteResource( resources, resource, _BROKER );
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_ExecQuery(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            const CMPIResult * results,          /** [out] Results of this operation. */
...
/// first loaded.
/// ----------------------------------------------------------------------------
CMPIStatus Linux_DHCPOptions_Initialize(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context)         /** [in] Additional context info, if any. */
{
    CMPIStatus status = {CMPI_RC_OK, NULL};
...
/** Create a new CMPIObjectPath to store this resource. */
        ...
        CMAddKey (op, "Name", (CMPIValue *)"OptionsIndication", CMPI_chars);
        CMAddKey (op, "SystemCreationClassName", (CMPIValue *)"CIM_ComputerSystem", CMPI_chars);
    
        instance = CMNewInstance( _BROKER, op, &status );
        CMSetProperty(instance, "Name", (CMPIValue *)"OptionsIndication", CMPI_chars);
        char *v;
        v = "SELECT * FROM Linux_DHCPOptions_Ind"; 
   ...
   //printf("Created an instance of OptionsIndication Filter\n");

    /** Initialize instance provider */
    ra_status = Linux_DHCPOptions_InstanceProviderInitialize( &ra_status );
    if ( ra_status.rc != RA_RC_OK ) {
...
/// ----------------------------------------------------------------------------
static CMPIStatus Linux_DHCPOptions_Cleanup(
            CMPIInstanceMI * self,               /** [in] Handle to this provider (i.e. 'self'). */
            const CMPIContext * context,         /** [in] Additional context info, if any. */
            CMPIBoolean terminating)             /** [in] Switch to wether to teminate or not*/ 
{
    ...
    ra_status = Linux_DHCPOptions_InstanceProviderCleanUp(lTerminating);
    if ( ra_status.rc != RA_RC_OK ) {
...
CMPIStatus Linux_DHCPOptions_IndicationCleanup(
                                           CMPIIndicationMI * mi,
...
CMPIStatus Linux_DHCPOptions_AuthorizeFilter(
                                           CMPIIndicationMI * mi,
                                           const CMPIContext * ctx,
...
CMPIStatus Linux_DHCPOptions_MustPoll(
                                    CMPIIndicationMI * mi,
                                    const CMPIContext * ctx,
                                    const CMPISelectExp * se,
...
CMPIStatus Linux_DHCPOptions_ActivateFilter(
                                          CMPIIndicationMI * mi,
                                          const CMPIContext * ctx,
                                          const CMPISelectExp * se,
                                          const char *ns,
...
CMPIStatus Linux_DHCPOptions_DeActivateFilter(
                                            CMPIIndicationMI * mi,
                                            const CMPIContext * ctx,
                                            const CMPISelectExp * se,
                                            const char *ns,
...
CMPIStatus Linux_DHCPOptions_EnableIndications(
                                              CMPIIndicationMI * mi,
                                              const CMPIContext * ctx)
{
    _enabled = 1;
...
CMPIStatus Linux_DHCPOptions_DisableIndications(
                                               CMPIIndicationMI * mi,
                                               const CMPIContext * ctx)
{
...
/// ============================================================================
CMInstanceMIStub( Linux_DHCPOptions_ , Linux_DHCPOptionsProvider, _BROKER, Linux_DHCPOptions_Initialize( &mi, ctx ) );
CMIndicationMIStub( Linux_DHCPOptions_ , Linux_DHCPOptionsProvider, _BROKER, CMNoHook );
